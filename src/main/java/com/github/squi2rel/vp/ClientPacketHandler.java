...
public class ClientPacketHandler {
        ...
        switch (type) {
            case CONFIG -> {
                ...
                if (!VideoPlayerClient.checkVersion(version)) {
                    Objects.requireNonNull(Minecraft.getInstance().player).displayClientMessage(Component.literal("服务器VideoPlayer版本和本地版本不匹配! 本地版本为" + VideoPlayerMain.version + ", 服务器版本为" + version), false);
                    return;
                }
            ...
            case EXECUTE -> {
                ...
                if (client.player != null) {
                    ClientCommandHandler.runCommand("vlc " + ByteBufUtils.readString(buf, 1024));
                }
            }
            case SET_UV -> {
                ClientVideoScreen screen = areas.get(readName(buf)).getScreen(readName(buf));
    ...
    private static void send(byte[] bytes) {
        VideoPayload.sendToServer(bytes);
    }
