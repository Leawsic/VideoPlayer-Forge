...
public interface IVideoPlayer {
    ...
    default void draw(PoseStack matrices, MultiBufferSource.BufferSource immediate, ClientVideoScreen s) {
        ...
        matrices.popPose();
        RenderType layer = ScreenRenderer.getLayer(getTextureId());
    ...
    default void draw(Matrix4f mat, VertexConsumer consumer, Vector3f p1, Vector3f p2, Vector3f p3, Vector3f p4, float u1, float v1, float u2, float v2) {
        ...
        consumer.vertex(mat, p1.x, p1.y, p1.z).uv(u1, v1).color(gray, gray, gray, 255).endVertex();
        consumer.vertex(mat, p2.x, p2.y, p2.z).uv(u1, v2).color(gray, gray, gray, 255).endVertex();
        consumer.vertex(mat, p3.x, p3.y, p3.z).uv(u2, v2).color(gray, gray, gray, 255).endVertex();
        consumer.vertex(mat, p4.x, p4.y, p4.z).uv(u2, v1).color(gray, gray, gray, 255).endVertex();
    }
}
