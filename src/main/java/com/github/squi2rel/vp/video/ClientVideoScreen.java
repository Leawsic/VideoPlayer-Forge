...
public class ClientVideoScreen extends VideoScreen {
    ...
    public void play(VideoInfo info) {
        ...
        if (source.isEmpty()) {
            ...
            player = VideoPlayers.from(info, this, null);
    ...
    public void autoSync(int clientDelay, long syncProgress) {
        ...
        if (player instanceof VideoPlayer vp && !vp.isPaused()) {
            ...
            syncFrames = Mth.clamp(syncFrames, 0, 7);
            ...
            if (syncFrames < 5) {
                if (delta > 10000) {
                    ...
                    vp.stop();
                    Minecraft.getInstance().gui.setOverlayMessage(Component.literal("提前太多，失去同步").withStyle(ChatFormatting.RED), false);
                }
            }
