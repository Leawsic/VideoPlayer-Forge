...
public class VideoPlayer implements IVideoPlayer, MetaListener {
    ...
    public synchronized void init() {
        ...
        decoder.onFinish(() -> Minecraft.getInstance().execute(() -> quad.resize(1, 1)));
    ...
    public void play(VideoInfo info) {
        if (targetTime > 0) {
            ...
            info = new VideoInfo(info.playerName(), info.name(), info.path(), info.rawPath(), info.expire(), info.seekable(), info.duration(), newParams);
        }
